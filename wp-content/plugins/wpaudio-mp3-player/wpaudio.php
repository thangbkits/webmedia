 ...
 * Author: Todd Iceton
 *
 * Copyright 2020 Todd Iceton (email: todd@wpaudio.com)
 *
...
if (!defined('WPAUDIO_DIR')) define('WPAUDIO_DIR', plugin_dir_path(__FILE__));
if (!defined('WPAUDIO_URL')) define('WPAUDIO_URL', plugins_url('', __FILE__));
...
function wpaLibraries(){
  global $wpa_version;
  $extension = WP_DEBUG === true ? '.js' : '.min.js';
  $url = WPAUDIO_URL . '/wpaudio' . $extension;
  wp_enqueue_script('wpaudio', $url, Array('jquery'), $wpa_version, true);
  # Add header action to include CSS and JS vars
  if (version_compare(get_bloginfo('version'), '4.5', '>=')) {
    wp_add_inline_script('wpaudio', wpaHeadContents(), 'before');
  } else {
    add_action('wp_head', 'wpaHead');
  }
}

function wpaHeadContents() {
  global $wpa_options;
  ...
  return "var _wpaudio = {url: '" . WPAUDIO_URL . "', enc: {}, convert_mp3_links: $wpa_pref_link_mp3, style: $style};";
}

## WPaudio style, jQuery, SWFObject
function wpaHead(){
  echo "<script type='text/javascript'>" . wpaHeadContents() . "</script>";
}
  ...
  if ($_POST) {
    ...
    foreach ($wpa_options as $key => $value) {
      if (isset($_POST[$key]) && !is_null($_POST[$key]) && $_POST[$key] !== '')
        $wpa_options[$key] = sanitize_text_field($_POST[$key]);
    }
    update_option('wpaudio_options', $wpa_options);
  }
  wpaOptions();
    ...
    <div class="postbox">
      ...
      <div class="inside">
        <ul>
          ...
          <li><label for="wpa_pref_link_mp3"><input name="wpa_pref_link_mp3" id="wpa_pref_link_mp3" type="checkbox" <?php if ($wpa_options['wpa_pref_link_mp3']) echo 'checked'; ?>>
    ...
    <div class="postbox">
      ...
      <div class="inside">
        <ul>
          ...
          <li><label for="wpa_tag_audio"><input name="wpa_tag_audio" id="wpa_tag_audio" type="checkbox" <?php if ($wpa_options['wpa_tag_audio']) echo 'checked'; ?>>
    ...
    <div class="postbox">
      ...
      <div class="inside">
        ...
        <ul class="wpa_style_advanced" style="display: none;">
          ...
          <li><label for="wpa_style_text_font"><input type="text" name="wpa_style_text_font" id="wpa_style_text_font" value="<?php echo esc_attr($wpa_options['wpa_style_text_font']); ?>"> Font face</label></li>
          <li><label for="wpa_style_text_size"><input type="text" name="wpa_style_text_size" id="wpa_style_text_size" value="<?php echo esc_attr($wpa_options['wpa_style_text_size']); ?>"> Font size</label></li>
          ...
          <li><label for="wpa_style_text_letter_spacing"><input type="text" name="wpa_style_text_letter_spacing" id="wpa_style_text_letter_spacing" value="<?php echo esc_attr($wpa_options['wpa_style_text_letter_spacing']); ?>"> Letter spacing</label></li>
        </ul>
        <ul class="wpa_style_advanced" style="display: none;">
          ...
          <li><label for="wpa_style_text_color"><input type="text" name="wpa_style_text_color" id="wpa_style_text_color" value="<?php echo esc_attr($wpa_options['wpa_style_text_color']); ?>" size="7"> Text color</label></li>
          <li><label for="wpa_style_link_color"><input type="text" name="wpa_style_link_color" id="wpa_style_link_color" value="<?php echo esc_attr($wpa_options['wpa_style_link_color']); ?>" size="7"> Link color</label></li>
          <li><label for="wpa_style_link_hover_color"><input type="text" name="wpa_style_link_hover_color" id="wpa_style_link_hover_color" value="<?php echo esc_attr($wpa_options['wpa_style_link_hover_color']); ?>" size="7"> Link hover color</label></li>
          <li><label for="wpa_style_bar_base_bg"><input type="text" name="wpa_style_bar_base_bg" id="wpa_style_bar_base_bg" value="<?php echo esc_attr($wpa_options['wpa_style_bar_base_bg']); ?>" size="7"> Bar base background</label></li>
          <li><label for="wpa_style_bar_load_bg"><input type="text" name="wpa_style_bar_load_bg" id="wpa_style_bar_load_bg" value="<?php echo esc_attr($wpa_options['wpa_style_bar_load_bg']); ?>" size="7"> Bar load background</label></li>
          <li><label for="wpa_style_bar_position_bg"><input type="text" name="wpa_style_bar_position_bg" id="wpa_style_bar_position_bg" value="<?php echo esc_attr($wpa_options['wpa_style_bar_position_bg']); ?>" size="7"> Bar position background</label></li>
        </ul>
      </div>
    </div>
  </div>
